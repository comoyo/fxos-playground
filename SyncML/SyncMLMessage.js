...
var syncMLMessage = function () {
  ...
  function cleanUpXML(xml) {
    ...
    xml2 = "";
    offset = 0;
    console.log("Trying to clean up xml");
  ...
  function readAnchor(node) {
    ...
    while (child) {
      ...
      case "Next":
        ...
        break;
      default:
        console.log("WARNING: readAnchor does not understand " + child.nodeName + ", yet. Value ignored. " + printNode(node));
        break;
      }
      child = child.nextSibling;
    }
  ...
  function readMeta(node) {
    ...
    while (child) {
      ...
      case "Size":
        ...
        break;
      default:
        console.log("WARNIG: readMeta does not understand " + child.nodeName + ", yet. Value ignored. " + printNode(node));
        break;
      }
      child = child.nextSibling;
    }
...
//    log("First child: " + child);
    while (child) {
      ...
      case "Source":
        ...
        break;
      default:
        console.log("WARNING: readHeader does not understand " + child.nodeName + ", yet. Value ignored. " + printNode(hdr));
        break;
      }
      child = child.nextSibling;
...
//          }
      ...
      case "Meta":
        ...
        break;
      default:
        console.log("Unexpected type (" + child.nodeName + ") in item received: " + printNode(child));
        break;
      }
  ...
  function readAlert(node) {
    ...
    while (child) {
      ...
      case "Item":
        ...
        break;
      default:
        console.log("WARNING: readAlert does not understand " + child.nodeName + ". Ignored. " + printNode(node));
        break;
      }
  ...
  function readStatus(node) {
    ...
    while (child) {
      ...
      case "Item":
        ...
        break;
      default:
        console.log("WARNING: readStatus does not understand " + child.nodeName + ". Ignored. " + printNode(node));
        break;
      }
  ...
  function readAddReplace(node) {
    ...
    while (child) {
      ...
      case "CmdRef":
        ...
        break;
      default:
        console.log("WARNING: readAddReplace does not understand " + child.nodeName + ", yet. Value ignored. " + printNode(node));
      }
      child = child.nextSibling;
    }
    if (!obj.cmdId) {
  ...
  function readSync(node) {
    ...
    while (child) {
      ...
      case "Add":
        ...
        if (obj) {
          console.log("Add an add.");
          sync.add.push(obj);
        } else {
          console.log("Skipped add: " + JSON.stringify(obj));
        }
        break;
      case "Replace":
        obj = readAddReplace(child);
        if (obj) {
          console.log("Add an replace.");
          sync.replace.push(obj);
        } else {
          console.log("Skipped replace: " + JSON.stringify(obj));
        }
        break;
      case "Delete":
        obj = readDelete(child);
        if (obj) {
          console.log("Add an delete.");
          sync.del.push(obj);
        } else {
          console.log("Skipped delete: " + JSON.stringify(obj));
        }
        break;
      case "CmdID":
      ...
      default: //ignore: NoResp, Cred, Meta and Atomic, Copy, Move, Sequence.
        console.log("Unexpected node type (" + child.nodeName + ") in sync received: " + printNode(node));
        break;
      }
  ...
  function readBody(body) {
    ...
    while (node) {
      ...
      case "Results":
        ...
        break;
      default:
        console.log("Unexpected node type (" + node.nodeName + ") in SyncBody received: " + printNode(node));
        break;
      }
  ...
  return {
    ...
    addCredentials: function (cred) {
      if (cred.encoded) { // allow to add encoded credentials directly
        ...
        if (cred.username && cred.password) {
          ...
          header.encodedCredentials = btoa(login);
        } else {
          console.log("Need cred.encoded or cred.username & cred.password. Please specify one of them.");
    ...
    buildMessageFromResponse: function (xml) {
      ...
      console.log("Cleaned up: " + cleanXML);
      ...
      if (errors.length > 0) {
        console.log("Had xml parse Errors, clean up xml and retry.");
      ...
      console.log("Header parsing finished.");
      ...
      console.log("Body parsing finished.");
    },
    ...
    matchCommandsFromMessage: function (cmds) {
      ...
      if (!body.status[msgRef]) { //wrong message ref... ?
        console.log("Wrong msgRef " + msgRef + " (might also mean that server did not send any status cmds.)");
        return undefined;
      }
        ...
        for (i = 0; i < obody.sync.length; i += 1) {
          ...
          if (cmdId || cmdId === 0) {
            if (body.status[msgRef][cmdId]) {
              ...
              for (ti = 0; ti < types.length; ti += 1) {
                for (j = 0; j < sync[types[ti]].length; j += 1) {
                  ...
                  if (body.status[msgRef][cmdId]) {
                    ...
                    console.log("Not status for cmd: " + JSON.stringify(cmd));
                  }
                }
              ...
              console.log("Sync cmd was not in this status, will skip all other sync-cmd-parts for this sync cmd.");
            }
          }
        }
      }
      ...
      console.log("Match commands finished.");
      return result;
    },
