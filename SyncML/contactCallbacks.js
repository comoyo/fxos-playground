//JSLint options:
/*global DB, console.log, console.log, vCard, SyncMLAccount, commonCallbacks */
  ...
  function updateContact(input) {
    ...
    var doUpdate = function (input, future) {
      try {
        ...
        console.log("Contact: " + JSON.stringify(c[0].name) + " with id: " + input.localId);
          ...
          function (f) {
            try {
              ...
              if (r.returnValue === true) {
                ...
                console.log("Callback not successfull: " + future.exception.errorText + "(" + future.exception.errorCode + "). :(" + input.item + JSON.stringify(input.localId));
                input.success = false;
              }
              if (input.callback) {
        ...
        //} else {
          console.log("ERROR: Could not convert " + input.item + " to contact!");
          input.success = false;
          if (input.callback) {
            input.callback(input);
          }
//        }
        ...
        console.log("Exception in UpdateContact(doUpdate): " + exception + " - " + JSON.stringify(exception) + " at " + input.item + " with ID " + input.localId);
        input.success = false;
        if (input.callback) {
          input.callback(input);
        }
    ...
    try {
      if (input.contact) {
        ...
        console.log("converting vCard to webOS data type:");
      ...
      console.log("Error in updateContact(main): ");
	...
	function applyContactHacks(contact, serverData) {
		var i, haveHome = false, haveWork = false, mail, problem = false, newMails = [], removeEmail = function (mail) {
			...
			if (mail.type === "type_home" && !haveWork) {
				...
				haveWork = true;
				newMails.push(mail);
				console.log("keep mail " + mail.value + " with changed type of " + mail.type);
				return;
			}
			...
			contact.note += "\nE-Mail: " + mail.value;
			console.log("Removed mail " + mail.value + " with type " + mail.type);
			console.log("new note: " + contact.note);
		};
		...
		if (contact.emails.length > 1) {
			console.log("Having more than one e-mails.");
			console.log("ServerMan: " + serverData.serverMan);
			if (serverData && serverData.serverMan  && serverData.serverMan.toLowerCase().indexOf("egroupware") > -1) {
				console.log("Server is egroupware. Check e-Mails. Server can only handle one work and one home address per contact!");
				for (i = 0; i < contact.emails.length; i += 1) {
					mail = contact.emails[i];
					console.log("Got mail " + mail.value + " of type " + mail.type);

					if (mail.type === "type_work") {
						console.log("work");
						if (haveWork) {
							console.log("had work already, kill this email.");
							removeEmail(mail);
						} else {
							console.log("keep this work mail.");
							newMails.push(mail);
							haveWork = true;
						}
						...
						console.log("home");
						if (haveHome) {
							console.log("had home already, kill this email.");
							removeEmail(mail);
						} else {
							console.log("keep this home mail.");
							newMails.push(mail);
							haveHome = true;
						}
					} else {
						console.log ("Unsupported type. Will try to change it.");
						if (!haveHome) {
							console.log("modified to home");
							mail.type = "type_home";
							haveHome = true;
							newMails.push(mail);
						} else if (!haveWork) {
							console.log("modified to work");
							mail.type = "type_work";
							haveWork = true;
							newMails.push(mail);
						} else {
		...
		if (problem) {
			console.log("now have " + newMails.length + " mails of " + contact.emails.length);
  ...
  function createContactArray(input) {
    ...
    var update = [], del = [], add = [], i, obj, result, callback, updates = 0,
			hackUpdateCallback = function (f) {
				var r = f.result;
				if (r.returnValue !== true) {
					console.log("Updated of modified contact failed.");
      ...
      try {
        if (future.result.returnValue === true) {
          this.data = future.result.result;
					if (this.noAdd === true) {
            console.log("Item was on server already, adding to update list: " + JSON.stringify(this.noAdd));
            update.push(this);
          } else {
            console.log("Item was not on server yet, adding to add list");
            if (this.contact) {
              console.log("Saving that this will be added to server.");
          ...
          updates -= 1;
          console.log("Remaining updates: " + updates);
          if (updates === 0) {
          ...
          console.log("Error in createContactArray(callback): ");
    ...
    try {
      if (input.success === true) {
        for (i = 0; i < input.result.length; i += 1) {
          try {
            ...
            console.log("Got contact: " + JSON.stringify(result));
            if (result._del === true) {
            ...
            console.log("Error while adding element " + i + " of " + input.result.length + ". Error: " + JSON.stringify(e));
            console.log(e);
          }
        }
      }
      ...
      console.log("Error in createContactArray(main): ");
    ...
    deleteContact: function (input) {
      ...
      commonCallbacks.deleteAllItems(input);
		...
		getNewContacts: function (input) {
		  console.log("Get new contacts called.");
