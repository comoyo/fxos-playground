//JSLint options:
/*global console.log, console.log, DB, Future, moboCopy */
...
var commonCallbacks = (function () {
  ...
  //will return public interface:
    ...
    getItemsFromDB: function(input) {
      try {
        console.log("Getting items: " + JSON.stringify(input.query));
        input.result = {};
        input.success = true;
          ...
          function (future) {
            ...
            try {
              ...
              if (r.returnValue === true) {
                ...
                console.log("Got " + r.results.length + " items from db.");
                input.success = true;
              } else {
                console.log("Error in getItemsFromDB: " + JSON.stringify(future.exception));
                input.success = false;
              }
              delete r.results; //try to release memory.
              input.callback(input);
            } catch (e) {
              console.log("Error in getItemsFromDB(Future): ");
        ...
        console.log("Error in getContactsFromDB(main): ");
		...
		deleteItem: function (input) {
		  console.log("Delete item called with id: " + input.localId);
			try {
              ...
              if (r.returnValue === true) {
                ...
                console.log("Callback not successfull: " + future.exception.errorText + "(" + future.exception.errorCode + " for itemId: " + input.localId + "). :(");
                input.success = false;
              }
              input.callback(input);
				    } catch (e) {
	            console.log("Error in deleteItem(Future): ");
	            console.log(JSON.stringify(e));
	            input.success = false;
	            input.callback(input);
	            console.log(e);
				...
				console.log("Exception in deleteItem: " + exception + " - " + JSON.stringify(exception));
				input.success = false;
        input.callback(input);
        console.log(exception);
			}
     ...
     * @param {Object} input = {kind: db.kind, account: account, datastore: datastore}
		...
		deleteAllItems: function (input) {
		  console.log("deleteAllItems called for " + input.kind + " from account " + input.account.accountId);
/*      try {
        DB.del({from: input.kind, where: [{prop: "accountId", op: "=", val: input.account.accountId}] }, false).then(this,
          function (future) {
            try {
              var r = future.result;
              if (r.returnValue === true) {
                console.log("Successfully deleted all elements: " + JSON.stringify(r));
                //input.callback({success: true});
                DB.put([{"_kind": input.kind, dummy: "dummy"}]).then(this, function (f) {
                    if (f.result.returnValue === true) {
                        console.log("did put dummy object. Id: " + f.result.results[0].id);
                        input.localId = f.result.results[0].id;
                        input.stats = { deleteOK: 0, delteFailed: 0};
                        this.deleteItem(input);
                    } else {
                        console.log("Could not put dummy object.");
                        console.log("Error in deleteAllItems: " + future.exception.errorText + " ( " + future.exception.errorCode + ")");
              ...
              console.log("Exception in deleteAllItems future: " + e + " - " + JSON.stringify(e));
              input.callback({success: false});
            }
          }
        );
			} catch (exception) {
				console.log("Exception in deleteAllItems: " + exception + " - " + JSON.stringify(exception));
      ...
      var outerFuture = new Promise();
		  console.log("getLatestRev called.");
			/*try {
				DB.find({from: kind,
            ...
            try {
              ...
              if (r.returnValue === true) {
                ...
                console.log("Error in getLatestRev: " + future.exception.errorText + "( " + future.exception.errorCode + ") = " + JSON.stringify(future.exception));
                outerFuture.result = { returnValue: false };
              }
            } catch (exception) {
              console.log("Exception in getLatestRev-Future: " + exception + " - " + JSON.stringify(exception));
              outerFuture.result = { returnValue: false };
            }
				  }
				);
			} catch (exception) {
				console.log("Exception in getLatestRev: " + exception + " - " + JSON.stringify(exception));
        outerFuture.result = { returnValue: false };
			}*/
      return outerFuture;
		},
      ...
      try {
        ...
        if (!datastore.ok || datastore.lastRev === 0) {
					if (!datastore.ok) {
						console.log("Sync had issues, resetting lastRev.");
					}
        ...
        console.log("Did " + stats.addOK + " adds, " + stats.addFailed + " adds failed.");
        console.log("Did " + stats.updateOK + " updates, " + stats.updateFailed + " updates failed.");
        console.log("Did " + stats.deleteOK + " deletes, " + stats.deleteFailed + " deletes failed.");
        console.log("Did " + datastore.addOwn + " adds on server");
        console.log("Did " + datastore.replaceOwn + " updates on server");
        console.log("Did " + datastore.delOwn + " deletes on server");
